/* Bachan's × Vesper Commerce — 1-hour delivery mockup interactions */

// ---------- Product catalog (homepage grid) ----------
const PRODUCTS = [
  {
    "line": "Sauce",
    "name": "The Original Japanese BBQ Sauce, .35oz",
    "img": "assets/p0.jpg",
    "price": "$1.00",
    "status": "",
    "rating": "",
...
// ---------- On-demand catalog (Bachan's Now page) ----------
const NOW_PRODUCTS = [
  {
    "line": "Sauce",
    "format": "Single",
...
// Bachan's Now address check -> reveal grid
function nowCheck() {
...
function setMode(mode) {
  ...
  if (addBtn && !isOneHr) {
    addBtn.textContent = mode === "subscribe" ? "Subscribe — $0.90" : "Add to cart — $1.00";
  }
}
function pdpCheck() {
...
const CO = { subtotal: 2.00, tax: 0.18, ship: 7.0, shipLabel: "1-Hour delivery" };
...
function placeOrder() {
  ...
  toast(oneHr ? "Order placed ⚡ Routing to nearest store — arriving in ~52 min"
              : "Order placed ✓ Thanks — your Bachan's order is confirmed.");
}
document.addEventListener("DOMContentLoaded", updateCheckoutTotals);

// ---------- Slide-out cart drawer + rewards meter ----------
const CART = {"name": "The Original Japanese BBQ Sauce, .35oz", "line": "Sauce", "img": "assets/p0.jpg", "unit": 1.0, "qty": 2, "sub": false};
const TIERS = [
...
function injectMobileMenu() {
  ...
  const active = location.pathname.endsWith("bachans-now.html") ? "brand-now" : "";
  const wrap = document.createElement("div");
  wrap.innerHTML = `
    <div class="menu-overlay" id="menuOverlay" onclick="closeMenu()"></div>
    <nav class="mobile-menu" id="mobileMenu" aria-label="Menu">
      <div class="mm-head">
        <span class="wordmark">Bachan's</span>
      ...
      <div class="mm-links">
        ...
        <a href="bachans-now.html" class="${active === "brand-now" ? "active" : ""}">Bachan's Now</a>
      </div>
      <div class="mm-cta"><a href="bachans-now.html" class="btn btn-white btn-block">⚡ Check your address</a></div>
      <div class="mm-sub">
